...
> = {
  ...
  booking: {
    lead: "この制作例には、実際に操作できる予約体験があります。文章で希望を送ると、営業時間・前後の準備時間・先約に照らして空き状況を判定し、取れないときは実際に空いている候補を出します。確定するのは、予約者が合意し、お店が承認した時だけです。変更を頼んでも、承認されるまで元の予約は残ります。",
    routes: [
      {
        href: "/daybook",
        label: "予約を相談してみる",
        detail: "例文4通り・候補の提示・仮押さえ・変更",
      },
      {
        href: "/daybook/admin",
        label: "運用側の台帳を見る",
        detail: "承認前後の差・2つの記録・他システムの予定",
      },
    ],
    note: "架空のスタジオです。実際の予約は入らず、メールも送られません。相談と予約はお使いのブラウザの中だけに保存されます。",
  },
  automation: {
