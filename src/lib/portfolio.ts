...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: "DAYBOOK",
    name: "予約管理ダッシュボード",
    summary: "「埋まっています」で終わらせない。空いている時間まで出す予約。",
    categories: ["apps", "nextjs", "responsive", "qa"],
    theme: "booking",
    brief:
      "小さなスタジオの予約を整理し、希望が埋まっているときのやりとりを減らしたい。",
    challenge:
      "「その時間は埋まっています」で止まり、空き探しのやりとりが何往復も続くこと。",
    solution:
      "文章で届いた希望から日付と時刻を読み取り、営業時間・前後の準備時間・先約に照らして判定します。取れないときは実際に空いている候補を出し、確定は予約者の合意とお店の承認がそろった時だけに限りました。",
    concept: "断るところで終わらず、次の一手まで出す。",
    features: [
      "文章から日時を読み取り、読み取れない項目は推測せず明示",
      "埋まっているときは、前後の準備時間まで踏まえた空き候補を提示",
      "予約者の合意とお店の承認、2つの記録がそろった時だけ確定",
      "変更は合意するまで元の予約を消さず、変わる所だけを表示",
    ],
    price: "要件により見積",
    duration: "要件確定後に提示",
    limitation:
      "架空のスタジオです。実際の予約は入らず、メールも送られません。相談と予約はこのブラウザの中だけに保存され、他の訪問者とは共有されません。空き判定と候補の並びは決まった規則による処理で、AIは使っていません。",
  },
  {
    ...common,
    slug: "improvement",
  ...
  },
  {
    ...
    slug: "csv",
    title: "CSV AUTOMATOR",
    name: "CSV加工から、毎週の集計まで",
    summary: "一度決めた列の意味を覚えて、二回目からは質問しない集計ツール。",
    categories: ["automation", "apps", "coding", "qa"],
    theme: "automation",
    brief: "毎週のCSV整理を簡単な操作に置き換えたい。",
    challenge:
      "毎週届く同じ形式のファイルに、毎週同じ説明をやり直していること。",
    solution:
      "列の対応・通貨・重複条件をルールとして保存し、次のファイルへ自動で当てました。列の順番が変わっただけなら質問せず、前になかった列と税区分のときだけ確認します。",
    concept: "業務の一工程を確実に軽くする。",
    features: [
      "列の意味をルールとして保存し、次回から自動で適用",
      "前回との差・変更の記録・読み取れなかった行の一覧",
      "読み取れない行をその場で直して再集計（元ファイルは変更しない）",
      "数式対策付きのCSV出力と、報告文にそのまま貼れる要約",
    ],
    price: "要件により見積",
    duration: "仕様確認後に提示",
    limitation:
      "自主制作のツールです。処理はすべてこのブラウザの中で行い、ファイルはサーバーへ送信しません。UTF-8 / 10,000行 / 50列まで。AIは使っておらず、増減の理由を断定することもしません。XLSXとPDFの出力は、内容を検証できる形で出せないため用意していません。",
  },
  {
    ...common,
    featured: false,
