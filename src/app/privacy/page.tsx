...
const sections = [
  ...
  },
  {
    ...
    title: "安全性と保存",
    body: [
      "入力途中の情報は、同じタブのsessionStorageへ一時保存します。2時間を超えた下書きは復元せず、受付成功時に削除します。端末を共有する場合は相談後にタブを閉じてください。下書きをサーバーへ保存したり、localStorageへ長期保存したりしません。案件サイト（CrowdWorks・ランサーズ・ココナラ）のリンクから開いた場合は、そのサービス名だけを同じタブに最大12時間記録し、相談とご契約をそのサービスのメッセージで進めていただく案内の表示に使います。この記録は計測に使わず、どこにも送信しません。",
      "受付番号・重複防止用の照合値・送信状態は、最長7日間の期限付きで管理します。受付通知とは別に、ご入力のメールアドレスへ自動受付メールをお送りします。",
      "Turnstileによるスパム検証を使用します。送信回数と重複判定は、ホスティング基盤に応じた保存領域（Cloudflare Durable Objects または Upstash）へ期限付きで保存し、本文・メールアドレス・生のIPアドレスは保存しません。メールとホスティング側のログは各サービスで管理されます。不要になった相談メールは運営者が削除します。削除の希望は返信メールまたはご利用中の案件サイトのメッセージからお知らせください。",
    ],
  },
  {
    id: "measurement",
    ...
    body: [
      "有効な場合、PostHogへ閲覧カテゴリ・作品・料金閲覧・相談操作と流入元の分類（crowdworks / lancers / coconala / direct / other）を送ります。タブ内で30分間の匿名IDを使います。本文・メールアドレス・URLクエリそのもの・永続ID・録画は送りません。DNT / GPCが有効なブラウザでは計測しません。",
