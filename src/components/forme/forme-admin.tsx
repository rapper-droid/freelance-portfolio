  ...
  return (
    <>
      <p className="forme-note">
        {
          "この画面は、同じ端末で行われた注文と在庫だけを扱います。実店舗の管理画面ではなく、運営側の操作がお客さま側の画面にどう反映されるかを見るためのものです。"
        }
      </p>
      ...
      {tab === "stock" && (
        <>
          ...
          <p className="forme-fineprint">
            {
              "数を変えると、商品ページの在庫表示とカートの上限がそのまま変わります。注文が入ると減り、取り消すと戻ります。"
            }
          </p>
          <div
      ...
      {tab === "catalogue" && (
        <>
          ...
          <p className="forme-fineprint">
            {
              "取り扱いを止めると、一覧から消え、カートに入っていた分は注文の直前に知らせます。価格は注文時点で控えているため、過去の注文金額は変わりません。"
            }
          </p>
          <div
            className="forme-table-scroll"
      ...
      {tab === "takings" && (
        <>
          ...
          <p className="forme-fineprint">
            {
              "取り消した注文は合計に含めていません。未収は、シミュレーターで拒否・保留・結果不明になった分と、まだ支払を試していない分です。決済は外部サービスへ接続していないため、実際の入金はありません。"
            }
          </p>
        </>
      )}
