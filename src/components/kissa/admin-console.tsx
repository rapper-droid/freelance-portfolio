  ...
  return (
    <>
      <p className="kissa-note">
        {
          "この画面は、同じ端末で行われた注文と予約だけを扱います。実店舗の運営端末ではなく、運営側の操作がお客さま側の画面にどう反映されるかを見るためのものです。"
        }
      </p>
      ...
      {tab === "menu" && (
        <>
          ...
          <p className="kissa-fineprint">
            {
              "ここで変えた状態は、メニューと商品ページにそのまま出ます。売り切れにした商品はカートに入れられなくなり、すでにカートにある場合は注文の直前に知らせます。"
            }
          </p>
          <div
            className="kissa-table-scroll"
      ...
      {tab === "takings" && (
        <>
          ...
          <p className="kissa-fineprint">
            {
              "取り消した注文は合計に含めていません。未収は、店頭でのお支払いや、シミュレーターで拒否・保留・結果不明になった分です。決済は外部サービスへ接続していないため、実際の入金はありません。"
            }
          </p>
        </>
      )}
