...
更新: 2026-09-21（A-04 出品原稿の確定後） / 対象 repo: `rapper-droid/freelance-portfolio`
...
| A-04 出品・応募・投稿・送信 | 出品3件 **READY**（所長が画面で公開）／応募・送信は未実行 |
...
## A-04 案件サイト・販売先への出品／応募 — 出品3件が READY（公開は所長の操作待ち）

所長の指示（2026-09-21）: **出品から着手。私が原稿・価格・納期・FAQ・画像まで
確定し、所長が画面で貼って公開ボタンを押す。** 価格は媒体でも自社サイトと同額、
手数料は自己負担。

| 出品                 | 媒体       | 原稿                                      | 表示価格 | 手数料 | 受取     | 納期 | 状態                |
| -------------------- | ---------- | ----------------------------------------- | -------- | ------ | -------- | ---- | ------------------- |
| 表示崩れの修正 1か所 | ココナラ   | `listings/coconala/service-web-fix.md`    | 5,500円  | 22%    | 4,290円  | 5日  | READY（所長が公開） |
| 表示崩れの修正 1か所 | ランサーズ | `listings/lancers/package-web-fix.md`     | 5,500円  | 16.5%  | 4,592円  | 7日  | READY（所長が公開） |
| CSV整形ルーチン 1本  | ランサーズ | `listings/lancers/package-csv-routine.md` | 16,500円 | 16.5%  | 13,777円 | 14日 | READY（所長が公開） |

手順は `artifacts/growth-public/listings/PUBLISH_CHECKLIST.md`（画面で踏む順に
1項目ずつ）。画像は `listings/images/`（`scripts/growth-listing-images.mjs` が
図と文字だけで生成。AI生成イラストなし・QRコードなし）。

規約の確認（2026-09-21 追加取得、`CHANNEL_POLICY.md` に全文の要約と出典）:

- **ココナラ: 自社サイトURLは禁止（VERIFIED）。** 問い合わせフォームを持つサイトの
  記載はNG例そのもの。URLなし・サイト名なし・検索誘導なしで原稿を確定した。
  実績はプロフィールのポートフォリオ欄に載せる。画像内のQRコードも禁止。
- **ランサーズ: 提案・パッケージへの外部URLは規定なし（UNVERIFIED）。** 細則と
  提案ガイドラインの全文を取得しても、禁止も許可も見つからなかった。第31条
  第1項第14号は「登録内容を明確にせず」の誘導を禁じるものなので、作業範囲を
  書いたうえでデモへのリンクを1つ添える形で運用する。
- **CrowdWorks: 応募・提案側の内容ルールは存在しない（UNVERIFIED）。** 仕事依頼
  ガイドラインはクライアントの依頼掲載のみを規律している。既定はURLなしの原稿。

| 項目       | 内容                                                                                                     |
| ---------- | -------------------------------------------------------------------------------------------------------- |
| 残っている | CrowdWorks の案件調査（SCOUT / GX-05、現在0件）と案件ごとの応募文、協業募集への連絡。いずれも1件ずつ承認 |
| 承認の単位 | 媒体・出品1件・宛先1件ごと。本文・リンク・価格・添付を見てから                                           |
| 公開後     | 3件のURLをこの表に追記し、閲覧数・問い合わせ数を `EXPERIMENTS.md` に記録する                             |
