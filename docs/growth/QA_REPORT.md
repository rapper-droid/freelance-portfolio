...
本番反映: **実施済み**。A-01（導線）= `ab261d93`、A-02（承認価格）= `6ce20d6f`、A-03（計測）= `5ec6e5ee`（現在稼働中）
...
| G32 | PASS      | 計測を接続（2026-09-21）。`/api/analytics` の 202/204/400/502 で「計測した・していない・不正・上流異常」を区別する     |
...
| G46 | N/A       | T02 未実装                                                                                                             |
| G47 | 一部 PASS | 接続済み。イベントは許可リストのみで本文・URL を含まない。E2E・診断由来の除外は集計開始時に設定する                    |
...
- 計測は 2026-09-21 に接続済み。ただし**データはこの日以降の分のみ**で、それ以前の流入は記録がない。
