...
更新: 2026-09-20 / このファイルだけで再開できるようにしてある。
...
1. **A-01 の承認 → 本番反映**（これをしないと新しい導線は誰にも見えない）
2. **候補の調査（GX-05）**: 協業募集の一次情報を集め、足切りと採点を通し、
   下書きを作る。CrowdWorks 案件は `npm run growth:brief -- --cw` で
   CW APPLY OS に渡す（この repo で二重管理しない）
3. **事例3本・解説ページ・完成品テンプレート（GX-06/07）**
