...
- `unit` … `npm run verify`（vitest）。34 ファイル / 397 テスト、exit 0。
- `e2e` … `tests/e2e/flow.spec.ts`（9 本 × 3 プロファイル = 27）。
...
| ------- | ---: | ----------------------- |
| PASS    |   67 |                         |
| FAIL    |    5 | A05, A10, R01, O07, U08 |
| BLOCKED |    4 | B06, O01, O02, O09      |
| N/A     |    4 | S01, S09, D05, D09      |
| 合計    |   80 |                         |

**BLOCKED と FAIL を PASS に数えていない。合計 9 項目が未完了。**
...
| A10 | FAIL | **体験→相談への引き継ぎが未実装。** `/flow` から `/contact` へリンクするが、選んだ困りごと・ツール・頻度を前埋めしない                 |
...
| U08 | FAIL | **`/demos/qa` への実テスト結果（SHA・日時）の反映は未実装。** 既存の手動チェック画面のまま                  |
...
### FAIL 5 件（実装すれば満たせるが、今回やっていない）

1. **A05** — `/demos/inbox` `/demos/admin` を共通 runtime へ統合していない。
   新しい 3 経路は 1 つの runtime を共有するが、旧デモは旧実装のまま残した
   （既存成果を壊さないことを優先）。
2. **A10** — 体験内容を相談フォームへ引き継いでいない。`/flow` から `/contact`
   へのリンクはあるが、選んだ困りごと・ツール・頻度を前埋めしないため再入力が残る。
3. **R01** — 実フォームの受付イベントから RELAY を起動していない。
   `/api/contact` は 1 行も変更していないため、実際の問い合わせは処理対象外。
4. **O07** — 停滞・接続期限切れを管理者へ知らせる仕組みがない。監視画面も無い。
5. **U08** — `/demos/qa` に実テスト結果（対象 SHA・実行日時）を反映していない。
