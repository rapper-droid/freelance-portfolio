...
- `unit` … `npm run verify`（vitest）。35 ファイル / 412 テスト、exit 0。
...
| ------- | ---: | ------------------ |
| PASS    |   68 |                    |
| FAIL    |    4 | A05, R01, O07, U08 |
| BLOCKED |    4 | B06, O01, O02, O09 |
| N/A     |    4 | S01, S09, D05, D09 |
| 合計    |   80 |                    |

**BLOCKED と FAIL を PASS に数えていない。合計 8 項目が未完了。**
...
4. **U08** — `/demos/qa` に実テスト結果（対象 SHA・実行日時）を反映していない。
