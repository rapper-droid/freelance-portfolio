...
> **2026-09-24 時点の更新。** この文書の D 節は 2026-09-23 に書いたもので、
> そのうち **U08（品質ページへの実テスト結果）と A05（旧デモの統合）は完了**した。
> いま所長の操作が要るのは次の 3 点だけ。
>
> 1. **P4・P5 の本番反映の承認。** tsudowa.com は P1〜P3 の Worker version
>    `46f3f849` のまま。手順は A-2 と同じ。
> 2. **実 AI の鍵**（B 節）。入るまで未稼働で、画面にもそう出る。
> 3. **実メール・実決済の有効化**。それぞれ別の承認。
>
> 実行しなかった操作（C 節）は P4・P5 でも同じ: 実メール 0 件、本番デプロイ 0 件、
> DNS 変更 0 件、force push 0 件、他 worktree・他ブランチの変更 0 件。
