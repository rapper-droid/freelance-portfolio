...
| **tetsuworks.com**（Netlify）                             | **全パス 301 → `https://tsudowa.com/works`**。中身はこのリリースそのもの |
...
実行時エラー 0
tetsuworks.com/            301 -> tsudowa.com/works（中身は上記と同一。実測で確認）
```
...
### tetsuworks.com について（2026-09-24 に実測して分かったこと）

**tetsuworks.com は古いビルドを配信していません。全パスが 301 で
`https://tsudowa.com/works` に飛びます**（`www` も apex 経由で同じ、Server は
Netlify）。だから tetsuworks.com を開いた人が見るのは、このリリースで反映した
ページそのものです。実測で DAYBOOK ×7・REPORT FLOW ×2・`/daybook`・`/report`
へのリンク・「操作できる版」×20 を確認しました。

このリダイレクトは**リポジトリの中にありません**。`netlify.toml` は
`codex/portfolio-sales-hub-v2` と現在のブランチで同一で、`[[redirects]]` も
`_redirects` も存在しません。Netlify のサイト／ドメイン設定側です。つまり:

- `codex/portfolio-sales-hub-v2` へ push しても、**訪問者が見るものは変わりません**。
  Codex 独自のコミットは 0 件（`ce03541` は現在のブランチの祖先）なので競合は
  ありませんが、走るのは誰も配信していないビルドです。
- しかも `scripts/check-production.mjs` は `NEXT_PUBLIC_SITE_URL` のホストが
  `tsudowa.com` でなければビルドを落とします。そのサイトの環境変数が
  tetsuworks.com なら、push の結果は赤いビルドだけです。

`docs/rebuild/REBUILD_BASELINE.md` は tetsuworks.com を「200 / 1.91s・別配信」と
記録しています。**現在は 301 なので、その後に誰かがリダイレクトを入れています。**
`~/.claude/CLAUDE.md` と Registry の「tetsuworks.com 稼働中・本番ビルド元は
`codex/portfolio-sales-hub-v2`」も、いまは実態とずれています（所長の資料なので
こちらでは直していません）。

所長の判断: **リダイレクトのまま**。パス保持への変更や独立配信への復帰は
していません。
